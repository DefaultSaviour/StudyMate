...
### Screen checklist
- [x] **Home / Dashboard** — main landing after login; wood bg, glass card with NEXT DUE + 4 nav buttons (Assignments, Flashcards, Calendar, Statistics), 6 orbs in wood band above, settings icon outside the card top-right. **Subjects is no longer a dashboard button** — it now lives inside the Assignments screen (mirrors how Flashcards owns its decks under one dashboard entry). The freed vertical space is reserved for a future sprint.
