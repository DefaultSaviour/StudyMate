...
### Screen checklist
- [x] **Home / Dashboard** — main landing after login; wood bg, glass card with NEXT DUE + 5 nav buttons, 6 orbs in wood band above, settings icon outside the card top-right
- [x] **Subjects list** — RecyclerView of glass-row items (colour dot + name + assignment count + edit/delete icons), single gold "Create subject" primary button, inline add + edit panel-swap, back-arrow icon outside card top-right
